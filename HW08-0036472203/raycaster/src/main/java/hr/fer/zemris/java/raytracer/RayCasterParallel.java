...
public class RayCasterParallel {
	...
	/**
	 * Return producer that renderes an image using ray-casting algorithm and
	 * parallelization benefits in order to accelerate pixel color
	 * determination.
	 * 
	...
	private static IRayTracerProducer getIRayTracerProducer() {
		return new IRayTracerProducer() {
			...
			public void produce(Point3D eye, Point3D view, Point3D viewUp,
				...
				/**
				 * This class represent a recursive job performed by threads
				 * independently.
				 * 
				 * @author Petra Rebernjak - 0036472203
				 *
				...
				class Job extends RecursiveAction {
					...
					/**
					 * Span treshold of the scene used as a recursion stopping
					 * point.
					 */
					private static final int SPAN_TRESHOLD = 16;
					...
					protected void compute() {
						if (end - start + 1 <= SPAN_TRESHOLD) {
							computeDirect();
						} else {
							invokeAll(
									new Job(start, start + (end - start) / 2),
									new Job(start + (end - start) / 2, end));
						}
					}

					/**
					 * Directly computes the colors of the part of the scene if
					 * that part of the image is small enough.
					 */
				...
				pool.invoke(new Job(0, height - 1));
				pool.shutdown();
			 ...
			 * intersect any objects in a scene, black color is set.
			 * 
			 * @param scene
			 *            The scene.
			 * @param ray
